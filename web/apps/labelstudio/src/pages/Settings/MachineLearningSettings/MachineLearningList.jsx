...
const BackendCard = ({ backend, onStartTrain, onEdit, onDelete, onTestRequest }) => {
  ...
  const confirmDelete = useCallback(
    (backend) => {
      confirm({
        title: t("delete_ml_backend"),
        body: t("delete_ml_backend_desc"),
    ...
    <Block name="backend-card">
      <div className={rootClass.elem("title-container")}>
        ...
        <div className={rootClass.elem("menu")}>
          <Dropdown.Trigger
            ...
            content={
              <Menu size="medium" contextual>
                ...
                <Menu.Item onClick={() => onTestRequest(backend)}>{t("send_test_request")}</Menu.Item>
                <Menu.Item onClick={() => onStartTrain(backend)}>{t("start_training")}</Menu.Item>
                <Menu.Divider />
          ...
          >
            <Button look="string" size="small" className="!p-0" aria-label={t("machine_learning_model_options")}>
              <IconEllipsis />
            </Button>
          </Dropdown.Trigger>
        </div>
      ...
      <div className={rootClass.elem("meta")}>
        ...
        <div className={rootClass.elem("group")}>
          <Tooltip title={format(parseISO(backend.created_at), "yyyy-MM-dd HH:mm:ss")}>
            <span>
              {t("created")} {formatDistanceToNow(parseISO(backend.created_at), { addSuffix: true })}
            </span>
          </Tooltip>
        </div>
      </div>
