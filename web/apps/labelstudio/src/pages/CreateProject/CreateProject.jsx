    ...
    >
      ...
      <div className="w-full flex flex-col gap-2">
        ...
        <TextArea
          ...
          placeholder={t("optional_project_description")}
          rows="4"
          style={{ minHeight: 100 }}
          value={description}
...
export const CreateProject = ({ onClose }) => {
  ...
  const steps = {
    name: <span className={tabClass.mod({ disabled: !!error })}>{t("project_name")}</span>,
    import: <span className={tabClass.mod({ disabled: uploadDisabled })}>{t("data_import")}</span>,
    config: t("labeling_setup"),
  };
        ...
        <Modal.Header>
          ...
          <Space>
            <Button
              ...
              onClick={onDelete}
              waiting={waiting}
              aria-label={t("cancel_project_creation")}
            >
              {t("cancel")}
            </Button>
            <Button
