...
const EmptyConfigPlaceholder = () => {
  ...
  return (
    <div className={configClass.elem("empty-config")}>
      <p>{t("empty_labeling_config")}</p>
      <p dangerouslySetInnerHTML={{__html: t("empty_labeling_config_desc", {doc_link: `<a href=\"https://labelstud.io/tags/\" target=\"_blank\" rel=\"noreferrer\">${t("read_documentation")}</a>`})}} />
    </div>
  );
};
  ...
  return (
    <div className={configClass.elem("labels")}>
      <form className={configClass.elem("add-labels")} action="">
        <h4>{tagname === "Choices" ? t("add_choices") : t("add_label_names")}</h4>
        <span>{t("add_labels_hint")}</span>
        <textarea
          name="labels"
          id=""
          cols="50"
        ...
        />
        <Button type="button" size="small" look="outlined" onClick={onAddLabels} aria-label={t("add_labels")}>
          {t("add")}
        </Button>
      </form>
...
const ConfigureColumn = ({ template, obj, columns }) => {
  ...
  const columnsList = useMemo(() => {
    const cols = (columns ?? []).map((col) => {
      return {
        value: col,
        label: col === DEFAULT_COLUMN ? t("imported_file") : `$${col}`,
      };
    });
    if (!columns?.length) {
      cols.push({ value, label: t("imported_file") });
    }
    cols.push({ value: "-", label: t("set_manually") });
    return cols;
  ...
  const fieldLabel = useMemo(() => {
    ...
    if (template.objects.length > 1) {
      return t("use_object_for_field", { objectName, fieldName });
    } else {
      const fromText = columns?.length > 0 && columns[0] !== DEFAULT_COLUMN ? ` ${t("from")} ${t("field")} ` : ` ${t("from")} `;

      return t("use_object_from_field", { objectName, field: fromText });
    }
  }, [obj, template.objects, columns, t]);
...
const Configurator = ({
  ...
  const extra = (
    <p className={configClass.elem("tags-link")}>
      {t("configure_labeling_interface")}
      <br />
        ...
        {disableSaveButton !== true && onSaveClick && (
          <Form.Actions size="small" extra={configure === "code" && extra} valid>
            ...
            <Button
              ...
              onClick={onSave}
              waiting={waiting}
              aria-label={t("save_configuration")}
            >
...
export const ConfigPage = ({
  ...
  React.useEffect(() => {
    ...
    fetchData();
  }, [columns, project, externalColumns, api]);
